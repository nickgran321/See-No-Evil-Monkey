...
struct NicksVisualizer: View {
    ...
    var body: some View {
        ...
        ZStack {
             ...
             LinearGradient(gradient: Gradient(colors: [.red, .purple]), startPoint: .bottom, endPoint: .top).edgesIgnoringSafeArea(.all)
            
            VStack {
                
                Text(signal.toggleValue ? "🙈" : "🐵").font(.custom("Helvetica", size: (signal.floatValue * 150)))
                    .padding(30)
